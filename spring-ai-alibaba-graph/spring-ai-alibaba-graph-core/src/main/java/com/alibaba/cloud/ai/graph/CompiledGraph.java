...
public class CompiledGraph {
    ...
    public RunnableConfig updateState(RunnableConfig config, Map<String, Object> values, String asNode)
        ...
        if (asNode != null) {
            nextNodeId = nextNodeId(asNode, branchCheckpoint.getState(), null);
        }
    ...
    private String nextNodeId(EdgeValue route, Map<String, Object> state, String nodeId, CompiledGraph.AsyncNodeGenerator generator) throws Exception {
        ...
        if (route.value() != null) {
            ...
            derefState.setNodeGenerator(generator);
     ...
     */
    public String nextNodeId(String nodeId, Map<String, Object> state, CompiledGraph.AsyncNodeGenerator generator) throws Exception {
        return nextNodeId(edges.get(nodeId), state, nodeId, generator);

    }

    public String getEntryPoint(Map<String, Object> state) throws Exception {
        return nextNodeId(stateGraph.getEntryPoint(), state, "entryPoint", null);
    }
    ...
    public class AsyncNodeGenerator<Output extends NodeOutput> implements AsyncGenerator<Output> {
        ...
        protected AsyncNodeGenerator(Map<String, Object> inputs, RunnableConfig config) {
            ...
            if (isResumeRequest) {
                ...
                this.contextState = new HashMap<>(startCheckpoint.getState());
                ...
                this.contextState = new HashMap<>(initializedState.data());
        ...
        public Data<Output> next() {
            ...
            try {
                ...
                NodeState nodeState = cloneState(currentState);
                nodeState.setNodeGenerator(this);
                future = action.apply(nodeState, config).thenApply(partialState -> {
                    try {
                        currentState = partialState;
                        nextNodeId = nextNodeId(currentNodeId, currentState, nodeState.getNodeGenerator());
